...
class milkzmqServer
{
   ...
   public:
    ...
    /// Restart epoch shared by all image threads.
    /** Increment this when a restart is requested; each image thread compares local
     *  epoch to this value to determine whether it should rebuild stream state.
     */
    static std::atomic_uint64_t m_restartEpoch;

    /// Request restart of all image thread inner loops.
    static void requestRestart();

    /// Get the current restart epoch.
    static uint64_t restartEpoch();
...
std::atomic_uint64_t milkzmqServer::m_restartEpoch{ 0 };

inline milkzmqServer::milkzmqServer()
{
...
inline void milkzmqServer::requestRestart()
{
    m_restartEpoch.fetch_add( 1, std::memory_order_relaxed );
}

inline uint64_t milkzmqServer::restartEpoch()
{
    return m_restartEpoch.load( std::memory_order_relaxed );
}

inline int milkzmqServer::argv0( const std::string &av0 )
{
    m_argv0 = av0;
    milkzmq_argv0 = m_argv0;  // set the global
...
inline std::string milkzmqServer::argv0()
{
    return m_argv0;
}

inline int milkzmqServer::imagePort( const int &imagePort )
{
...
inline int milkzmqServer::imagePort()
{
    return m_imagePort;
}

inline int milkzmqServer::shMemImName( const std::string &name )
{
    s_imageThread nt;
...
inline size_t milkzmqServer::numImages()
{
    return m_imageThreads.size();
}

inline std::string milkzmqServer::shMemImName( size_t n )
{
    return m_imageThreads[n].m_imageName;
}

inline int milkzmqServer::usecSleep( const int &usec )
{
    m_usecSleep = usec;
    return 0;
}

inline int milkzmqServer::usecSleep()
{
    return m_usecSleep;
}

inline int milkzmqServer::fpsTgt( const float &fps )
{
    m_fpsTgt = fps;
    return 0;
}

inline float milkzmqServer::fpsTgt()
{
    return m_fpsTgt;
}

inline int milkzmqServer::fpsGain( const float &gain )
{
    m_fpsGain = gain;
    return 0;
}

inline float milkzmqServer::fpsGain()
{
    return m_fpsGain;
}

inline void milkzmqServer::noCompression()
{
...
inline void milkzmqServer::defaultCompression()
{
...
inline int milkzmqServer::xrifDifferenceMethod( const int &xdm )
{
    m_xrifDifferenceMethod = xdm;
    return 0;
}

inline int milkzmqServer::xrifDifferenceMethod()
{
    return m_xrifDifferenceMethod;
}

inline int milkzmqServer::xrifReorderMethod( const int &xrm )
{
    m_xrifReorderMethod = xrm;
    return 0;
}

inline int milkzmqServer::xrifReorderMethod()
{
    return m_xrifReorderMethod;
}

inline int milkzmqServer::xrifCompressMethod( const int &xcm )
{
    m_xrifCompressMethod = xcm;
    return 0;
}

inline int milkzmqServer::xrifCompressMethod()
{
...
inline void milkzmqServer::serverThreadExec()
{
    ...
    while( !m_timeToDie.load( std::memory_order_relaxed ) )  // loop on timeToDie in case this gets interrupted by SIGSEGV/SIGBUS
    {
...
// Wait for next request from a client
        ...
        {
            if( m_timeToDie.load( std::memory_order_relaxed ) )
                break;  // If an exception is thrown we check for timeToDie
            throw;
        }
...
inline int milkzmqServer::serverThreadKill()
{
    m_timeToDie.store( true, std::memory_order_relaxed );
    if( m_server )
        m_server->close();
...
inline int milkzmqServer::imageThreadKill( size_t thno )
{
    static_cast<void>( thno );
    m_timeToDie.store( true, std::memory_order_relaxed );
    return 0;
}

inline void milkzmqServer::imageThreadExec( const std::string &imageName )
{
    IMAGE image;
    ...
    uint64_t localRestartEpoch = restartEpoch();
    ...
    while( !m_timeToDie.load( std::memory_order_relaxed ) )
    {
        ...
        while( !opened && !m_timeToDie.load( std::memory_order_relaxed ) && restartEpoch() == localRestartEpoch )
        {
            ...
            {
                if( !printed )
                    reportWarning( "ImageStream " + imageName + " not found (yet).  Retrying . . ." );
                printed = 1;
        ...
        if( !opened )
        {
            localRestartEpoch = restartEpoch();
            continue;
        }
        ...
        while( !m_timeToDie.load( std::memory_order_relaxed ) && restartEpoch() == localRestartEpoch )
        {
            ...
            if( cnt0 != lastCnt0 )
            {
                ...
                if( m_timeToDie.load( std::memory_order_relaxed ) || restartEpoch() != localRestartEpoch )
                ...
                if( image.md[0].size[2] > 0 )  ///\todo change to naxis?
                {
                    ...
                    if( curr_image < 0 )
                        curr_image = image.md[0].size[2] - 1;
                }
                else
                    curr_image = 0;
                ...
                // XRIF Encoding...
                // Because of xrif->compress_on_raw == true, xrif->raw_buffer = msg + headerSize, this results in the encoded message being written to
                // the message buffer.
...
inline void milkzmqServer::reportInfo( const std::string &msg )
{
    milkzmq::reportInfo( m_argv0, msg );
}

inline void milkzmqServer::reportNotice( const std::string &msg )
{
    milkzmq::reportNotice( m_argv0, msg );
}

inline void milkzmqServer::reportWarning( const std::string &msg )
{
    milkzmq::reportWarning( m_argv0, msg );
}

inline void milkzmqServer::reportError( const std::string &msg, const std::string &file, int line )
{
