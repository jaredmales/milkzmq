...
void sigTermHandler( int signum, siginfo_t* siginf, void* ucont )
{
    ...
    milkzmq::milkzmqServer::m_timeToDie.store( true, std::memory_order_relaxed );
}

void sigSegvHandler( int signum, siginfo_t* siginf, void* ucont )
{
    // Suppress those warnings . . .
    static_cast<void>( signum );
    static_cast<void>( siginf );
    ...
    milkzmq::milkzmqServer::requestRestart();
}
...
int main( int argc, char** argv )
{
    ...
    for( ; n < static_cast<int>( argc - optind + streams.size() ); n++ )
        mzs.imageThreadStart( n );
    ...
    if( exportAll )
    {
        ...
        t_watcher = std::thread(
            [inotify_fd, &n, &mzs, shmDir]()
            {
                ...
                while( true )
                {
                    ...
                    if( bytes_read <= 0 )
                    {                                         // Check to see we actually got something.
                    ...
                    else
                    {
                        ...
                        for( int i = 0; i < num; i++ )
                        {                                              // Try and process one or more events.
                            struct inotify_event* ie = event_buf + i;  // Get the event.
                            if( !( ie->mask & IN_CREATE ) )
                                continue;  // Don't both if it isn't a create event.
                            if( ie->len <= 0 )
                                continue;                            // If we don't have a file name, try another.
                            std::filesystem::path path( ie->name );  // Make a path for the created file.
                            if( boost::algorithm::ends_with( path.filename().generic_string(), ".im.shm" ) )
    ...
    while( !milkzmq::milkzmqServer::m_timeToDie.load( std::memory_order_relaxed ) )
        milkzmq::sleep( 1 );

    // Stop everything.
    mzs.serverThreadKill();
    for( size_t m = 0; m < n; ++m )
        mzs.imageThreadKill( m );
