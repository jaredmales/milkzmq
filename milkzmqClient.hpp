...
inline milkzmqClient::milkzmqClient()
{
...
inline int milkzmqClient::argv0( const std::string &av0 )
{
    m_argv0 = av0;
    milkzmq_argv0 = m_argv0;  // set the global
    return 0;
...
inline std::string milkzmqClient::argv0()
{
    return m_argv0;
}

inline int milkzmqClient::address( const std::string &add )
{
    m_address = add;
    return 0;
}

inline std::string milkzmqClient::address()
{
    return m_address;
}

inline int milkzmqClient::imagePort( const int &imagePort )
{
...
inline int milkzmqClient::imagePort()
{
    return m_imagePort;
}

inline int milkzmqClient::shMemImName( const std::string &name )
{
    return shMemImName( name, name );
}

inline int milkzmqClient::shMemImName( const std::string &name, const std::string &localName )
{
    s_imageThread nt;
...
inline std::string milkzmqClient::shMemImName( size_t imno )
{
...
inline std::string milkzmqClient::localShMemImName( size_t imno )
{
    if( imno >= m_imageThreads.size() )
        return "";
...
inline void milkzmqClient::imageThreadExec( const std::string &imageName, const std::string &localImageName )
{
    ...
    while( !m_timeToDie.load( std::memory_order_relaxed ) )
    {
        ...
        while( !m_timeToDie.load( std::memory_order_relaxed ) && !reconnect )  // Inner loop waits for each new image and processes it as it comes in.
        {
            zmq::message_t msg;
            ...
            if( msg.size() < imageOffset + xrif->compressed_size )
            {
                reportWarning( "Received truncated frame for " + imageName + ", reconnecting." );
                reconnect = true;
                continue;
            }
...
inline int milkzmqClient::imageThreadKill( size_t thno )
{
...
inline void milkzmqClient::reportInfo( const std::string &msg )
{
    milkzmq::reportInfo( m_argv0, msg );
}

inline void milkzmqClient::reportNotice( const std::string &msg )
{
    milkzmq::reportNotice( m_argv0, msg );
}

inline void milkzmqClient::reportWarning( const std::string &msg )
{
    milkzmq::reportWarning( m_argv0, msg );
}

inline void milkzmqClient::reportError( const std::string &msg, const std::string &file, int line )
{
